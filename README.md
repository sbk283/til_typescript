...
- 만약 1개의 객체를 생성하는 경우라면 이름은 `카멜 케이스`로 한다.
...
- 여러개의 객체를 생성하는 경우는 `파스칼 케이스` 를 함수명으로 지정.
```js
function PersonInfo () {
    this.name = "홍길동",
    this.age = 20,
...
```
- 많이 복잡합니다. 추후 필요로 하는것은 class 라는 문법입니다.
```ts
...
```
- 아래형태를 더 추천함. type을 추가하면 가독성이 좋다.
```ts
type PersonInfo = {
    name: string;
    age: number;
...
function PersonInfo (this: PersonInfo) {
    this.name = "홍길동",
    this.age = 20,
    this.job = "소설가"
}
```
- 절대 주의해야 합니다. (객체 생성하고 싶은 경우 `new` 를 꼭 붙인다.)
```js
function PersonInfo () {
    this.name = "홍길동",
    this.age = 20,
...
function PersonInfo (this: PersonInfoType): void {
    ...
    this.age = 20,
    this.job = "소설가"
    // Default 로 this 가 리턴됨
    // return this;
}

// ts 에서는 new 는 class 로 생성을 원활히 지원된다.
// 객체 생성자 함수를 ts로 하려면 편법을 써서 할수밖에 없다.
...
### 3.2. `new`를 붙여서 함수 사용한다면 객체를 생성 하겠다는 목적
```js
function PersonInfo () {
    this.name = "홍길동",
    this.age = 20,
...
블랙핑크_멤버_지수.이름; // 지수
블랙핑크_멤버_지수["이름"] // 지수
const 블랙핑크_멤버_리사 = {이름: "지사", 생일: "97-03"};
...
const 블랙핑크_멤버_제니: {이름: string, 생일: string} = {이름: "제니", 생일: "96-01"};
const 블랙핑크_멤버_로제: {이름: string, 생일: string} = {이름: "로제", 생일: "97-02"};
const 블랙핑크_멤버_지수: {이름: string, 생일: string} = {이름: "지수", 생일: "95-01"};
블랙핑크_멤버_지수.이름; // 지수
블랙핑크_멤버_지수["이름"] // 지수
const 블랙핑크_멤버_리사: {이름: string, 생일: string} = {이름: "리사", 생일: "97-03"};
...
## 6. 객체에 기능 추가하기
- 객체에 만든 `함수`를 `Method, Behavior(행동)` 이라고 함
...
const 블랙핑크_멤버_제니 = {
    ...
    sing: function () {
        // 기능(Method)
        console.log(`${this.이름}가 노래 합니다.`);
    },
    dance: function () {
        console.log(`${this.이름}가 춤을 춥니다.`);
    },
};
블랙핑크_멤버_제니.sing(); // 제니가 노래 합니다. 출력
블랙핑크_멤버_제니.dance(); // 제니가 춤을 춥니다. 출력
```
...
- `typescript` 에서는 `class` 를 쓰는게 편하다.
...
### 6.3. 객체에 속해있는 메서드를 축약해서 생성하는 문법
...
## 7. 객체를 복사하기
- 값의 복사 (string, number, null, undefined etc..)
```js
const age = 10;
// age = 12; // const 에러 발생
console.log(age);
...
아이유 = "10살"; // const 는 불변하다. 아이유 = {job: "배우"}; // const 는 재할당이 불가능하다.

아이유.job = "배우"; // const 는 불변하지만, 객체의 속성은 변경 가능하다.

const 아이돌 = 아이유; // const 는 재할당이 불가능하지만, 참조는 가능하다.
...
- 객체는 반드시 아래처럼 복사해야한다.
```js
const 아이유 = {job: "가수", age: 10, song: "좋은 날"};
